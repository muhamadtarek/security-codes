...
def ascii_to_binary(input_string, bit_length):
    binary_string = ''.join(format(ord(char), '08b') for char in input_string)
    return binary_string[:bit_length].ljust(bit_length, '0')
...
def userinput():
    keyinp = input("Enter the key as an ASCII string (up to 7 characters): ")
    datainp = input("Enter the data as an ASCII string (up to 8 characters): ")

    binary_key = ascii_to_binary(keyinp, 56)
    binary_data = ascii_to_binary(datainp, 64)

    return list(map(int, binary_key)), list(map(int, binary_data))
...
def main():
    ...
    encrypted_data = permutation(data, 1)
    print("Encrypted data is:", ''.join(str(bit) for bit in encrypted_data))
    print("Time taken to encrypt the data with DES is", time.time() - starttime)

    # Decryption
    startdecrypt = time.time()
    encrypted_data = permutation(encrypted_data, 0)
    for i in range(16):
        encrypted_data = round(encrypted_data, key16[15 - i])
    encrypted_data = np.roll(encrypted_data, 32)
    decrypted_data = permutation(encrypted_data, 1)
    ascii_decrypted = binary_to_ascii(''.join(str(bit) for bit in decrypted_data))
    print("Decrypted ASCII data is:", ascii_decrypted)
    print("Time taken to encrypt the data with DES is", time.time() - startdecrypt)
